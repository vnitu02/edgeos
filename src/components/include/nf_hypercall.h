...
nf_hyp_malloc(size_t size, vaddr_t *vaddr)
{
       ...
       return cos_sinv_rets(BOOT_CAPTBL_FREE, 0, NF_MALLOC, size, 0, vaddr, &unused);
}
...
nf_hyp_confidx_get(int *conf_file_idx)
{
       ...
       return cos_sinv_rets(BOOT_CAPTBL_FREE, 0, NF_CONFIDX_GET, 0, 0, (word_t *)conf_file_idx, &unused);
}
